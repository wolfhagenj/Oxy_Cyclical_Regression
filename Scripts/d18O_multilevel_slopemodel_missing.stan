data {
    ...
    int<lower = 1> N_Samples_Missing; //observed dist values to estimate the d18O value
    //clustering categories
    array[N_Samples] int<lower = 0> Specimen;
    array[N_Samples_Missing] int<lower = 0> Specimen_Missing;
    //observations
    ...
    array[N_Samples_Missing] real<lower = 0> dist_obs_missing;
    array[N_Samples_Missing] real<lower = 0> dist_sd_missing;
    //hyper-parameters
    array[2] real<lower = 0> prior_period;
    real<lower = 0> period_low;
    real<lower = period_low> period_high;
    array[2] real<lower = 0> prior_sigmasample_O;
}
parameters {
    //observations
    vector[N_Samples] d18O;
    vector<lower = 0>[N_Samples] dist;
    vector<lower = 0>[N_Samples_Missing] dist_missing;
...
model {
    ...
    //sampling variability
    sigmasample_O ~ normal(prior_sigmasample_O[1], prior_sigmasample_O[2]);
    //observation model
    dist_obs ~ normal(dist, dist_sd);
    dist_obs_missing ~ normal(dist_missing, dist_sd_missing);
...
generated quantities {
  ...
  vector[N_Samples_Missing] d18O_missing;
  //predicting d18O values for removed (missing) values
  for(i in 1:N_Samples_Missing) {
    real d18O_mu_missing;
    //matches formula in Chazin, et al. (2018) Formula 3.2 (with slope term added) rather than Stolwijk, et al. (1999)
    d18O_mu_missing = (alpha_oxy[Specimen_Missing[i]] + beta_oxy[Specimen_Missing[i]] * dist_missing[i]) + Obeta1[Specimen_Missing[i]] * cos(2 * pi() * (dist_missing[i] / period_oxy[Specimen_Missing[i]])) + Obeta2[Specimen_Missing[i]] * sin(2 * pi() * (dist_missing[i] / period_oxy[Specimen_Missing[i]]));
    //sampling varibaility
    d18O_missing[i] = normal_rng(d18O_mu_missing, sigmasample_O);
  }
